...
print( '' )
print( 'Running PlotFieldsHDF_2D.py...' )
print( '------------------------------' )
...
RootPath = '/Users/dunhamsj/'
suffix = 'Desktop/'

Problem = 'Advection'

xL = np.array( [ 0.0, 0.0, 0.0 ], np.float64 )
xU = np.array( [ 1.0, 1.0, 1.0 ], np.float64 )
...
for i in range( nFiles ):
    ...
    if PlotTranspose:
        ...
        Data[i] \
          = np.abs( names[Field][1][i,0,:,:] - names[Field][1][i,0,:,:].T )

    else:

        Data[i] = names[Field][1][i,0,:,:]
...
if CoordinateSystem == 'SPHERICAL':
    ...
    if PlotContours:
        ...
        im = ax.pcolormesh( X1v, X2v, Data[0,0], \
                            cmap    = cmap, \
                            norm    = Norm, \
                            shading = 'nearest' )
